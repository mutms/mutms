...
use mod_mubook\event\content_updated;
 ...
 *
 * @covers \mod_mubook\event\content_updated;
 */
final class content_updated_test extends \advanced_testcase {
    ...
    public function test_event(): void {
        ...
        $content->update((object)['id' => $content->id, 'text' => 'abc']);
        $events = $sink->get_events();
        $sink->close();

        $this->assertCount(1, $events);
        $this->assertInstanceOf(content_updated::class, $events[0]);

        $event = $events[0];
        $this->assertEquals($context->id, $event->contextid);
        $this->assertSame($content->id, $event->objectid);
        $this->assertSame('u', $event->crud);
        $this->assertSame($event::LEVEL_TEACHING, $event->edulevel);
        $this->assertSame('mubook_content', $event->objecttable);
        $this->assertSame('Content updated', $event::get_name());
