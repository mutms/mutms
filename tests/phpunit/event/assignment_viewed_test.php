...
/**
 * Certification assignment viewed event test.
 *
 * @group      MuTMS
 ...
 *
 * @covers \tool_mucertify\event\assignment_viewed
 */
final class assignment_viewed_test extends \advanced_testcase {
    ...
    public function test_unassign_user(): void {
        ...
        $program = $programgenerator->create_program(['sources' => 'mucertify', 'viewed' => 1]);
        ...
        $event = reset($events);
        $this->assertInstanceOf(\tool_mucertify\event\assignment_viewed::class, $event);
        ...
        $this->assertSame('Certification assignment viewed', $event::get_name());
        $description = $event->get_description();
        $certificationurl = new \core\url('/admin/tool/mucertify/my/certification.php', ['id' => $assignment->certificationid, 'userid' => $assignment->userid]);
