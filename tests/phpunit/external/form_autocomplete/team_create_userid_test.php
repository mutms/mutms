 ...
 *
 * @coversDefaultClass \tool_murelation\external\form_autocomplete\team_create_userid
 */
final class team_create_userid_test extends \advanced_testcase {
    ...
    public function test_execute(): void {
        ...
        $result = team_create_userid::execute('', $framework1->id, null);
        ...
        $result = team_create_userid::execute('irst', $framework1->id, 0);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user1->id);

        $result = team_create_userid::execute('', $framework2->id, null);
        $this->assertFalse($result['overflow']);
        $this->assertCount(2, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user1->id);
        $this->assertSame($result['list'][1]['value'], $user2->id);

        $result = team_create_userid::execute('user2@', $framework2->id, null);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user2->id);

        try {
            team_create_userid::execute('', $framework0->id, null);
        ...
        $this->setUser($user1);
        try {
            team_create_userid::execute('', $framework1->id, null);
            $this->fail('exception expected');
        } catch (\core\exception\moodle_exception $ex) {
            $this->assertInstanceOf(\invalid_parameter_exception::class, $ex);
        ...
        $result = team_create_userid::execute('', $framework1->id, $tenant1->id);
        $this->assertFalse($result['overflow']);
        ...
        $result = team_create_userid::execute('', $framework1->id, $tenant2->id);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user2->id);
    }
    ...
    public function test_format_label(): void {
        ...
        $this->setUser($user);
        $result = team_create_userid::format_label($user1, $syscontext);
    ...
    public function test_validate_value(): void {
        ...
        $this->assertSame(null, team_create_userid::validate_value(
            $user1->id,
        ...
        $this->assertSame('Error', team_create_userid::validate_value(
            $user0->id,
            ['frameworkid' => $framework2->id, 'tenantid' => null],
            $syscontext
        ));
