...
$currenturl = new moodle_url('/admin/tool/mucertify/management/source_approval_delete.php', ['id' => $id]);
$PAGE->set_context($context);
$PAGE->set_url($currenturl);

$returnurl = new moodle_url('/admin/tool/mucertify/management/source_approval_requests.php', ['id' => $certification->id]);

$form = new \tool_mucertify\local\form\source_approval_delete(null, ['request' => $request, 'user' => $user, 'certification' => $certification, 'context' => $context]);
...
if ($data = $form->get_data()) {
    tool_mucertify\local\source\approval::delete_request($request->id);
