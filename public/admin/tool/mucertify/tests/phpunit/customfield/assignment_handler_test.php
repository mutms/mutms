 ...
 *
 * @covers \tool_mucertify\customfield\assignment_handler
 */
final class assignment_handler_test extends \advanced_testcase {
    ...
    public function test_assign_users(): void {
        ...
        assign_capability('tool/mucertify:assign', CAP_ALLOW, $managerroleid, $catcontext);
        ...
        $data1 = $DB->get_record('customfield_data', ['instanceid' => $assignment1->id, 'fieldid' => $field1->get('id')], '*', MUST_EXIST);
        ...
        $data2 = $DB->get_record('customfield_data', ['instanceid' => $assignment1->id, 'fieldid' => $field2->get('id')], '*', MUST_EXIST);
    ...
    public function test_assignment_update(): void {
        ...
        assign_capability('tool/mucertify:admin', CAP_ALLOW, $cetadminroleid, $catcontext);
        ...
        $assignment1 = \tool_mucertify\local\source\base::assignment_update($data);
        $data1 = $DB->get_record('customfield_data', ['instanceid' => $assignment1->id, 'fieldid' => $field1->get('id')], '*', MUST_EXIST);
        $this->assertSame('Test value 1', $data1->charvalue);
        $this->assertSame($data1->charvalue, $data1->value);
        $this->assertSame((string)$catcontext->id, $data1->contextid);
    ...
    public function test_visibility(): void {
        ...
        assign_capability('tool/mucertify:admin', CAP_ALLOW, $adminroleid, $catcontext);

        $manager = $this->getDataGenerator()->create_user();
        role_assign($managerroleid, $manager->id, $catcontext->id);
        $certadmin = $this->getDataGenerator()->create_user();
        role_assign($adminroleid, $certadmin->id, $catcontext->id);
        $assigned1 = $this->getDataGenerator()->create_user();
        $assigned2 = $this->getDataGenerator()->create_user();
        ...
        $field2 = $this->getDataGenerator()->create_custom_field([
            ...
            'type' => 'text',
            'categoryid' => $fieldcategory->get('id'),
            'configdata' => ['visibilityassignee' => 1],
        ]);
        ...
        $this->setGuestUser();
        $datas = $handler->get_instance_data($assignment1->id);
