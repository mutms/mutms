...
/**
 * List of subordinate candidates for bulk subordinate creation tests.
 *
 * @group       MuTMS
 ...
 *
 * @coversDefaultClass \tool_murelation\external\form_autocomplete\subordinates_create_subuserids
 */
final class subordinates_create_subuserids_test extends \advanced_testcase {
    ...
    public function test_execute(): void {
        ...
        $result = subordinates_create_subuserids::execute('', $framework1->id, null, $user0->id);
        ...
        $result = subordinates_create_subuserids::execute('irst', $framework1->id, 0, $user0->id);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user1->id);

        $result = subordinates_create_subuserids::execute('', $framework2->id, null, $user0->id);
        $this->assertFalse($result['overflow']);
        $this->assertCount(2, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user1->id);
        $this->assertSame($result['list'][1]['value'], $user2->id);

        $result = subordinates_create_subuserids::execute('user2@', $framework2->id, null, $user0->id);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
        $this->assertSame($result['list'][0]['value'], $user2->id, $user0->id);

        try {
            subordinates_create_subuserids::execute('', $framework0->id, null, $user0->id);
        ...
        $this->setUser($user1);
        try {
            subordinates_create_subuserids::execute('', $framework1->id, null, $user0->id);
            $this->fail('exception expected');
        } catch (\core\exception\moodle_exception $ex) {
            $this->assertInstanceOf(\invalid_parameter_exception::class, $ex);
        ...
        $result = subordinates_create_subuserids::execute('', $framework1->id, $tenant1->id, $user0->id);
        $this->assertFalse($result['overflow']);
        ...
        $result = subordinates_create_subuserids::execute('', $framework1->id, $tenant2->id, $user0->id);
        $this->assertFalse($result['overflow']);
        $this->assertCount(1, $result['list']);
    ...
    public function test_format_label(): void {
        ...
        $this->setUser($user);
        $result = subordinates_create_subuserids::format_label($user1, $syscontext);
    ...
    public function test_validate_value(): void {
        ...
        $this->assertSame(null, subordinates_create_subuserids::validate_value(
            $user1->id,
        ...
        $this->assertSame('Error', subordinates_create_subuserids::validate_value(
            $user0->id,
            ['frameworkid' => $framework2->id, 'tenantid' => null, 'supuserid' => $user0->id],
            $syscontext
        ));
