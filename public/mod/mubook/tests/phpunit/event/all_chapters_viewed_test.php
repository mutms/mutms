...
use mod_mubook\event\all_chapters_viewed;
 ...
 *
 * @covers \mod_mubook\event\all_chapters_viewed;
 */
final class all_chapters_viewed_test extends \advanced_testcase {
    ...
    public function test_event(): void {
        ...
        all_chapters_viewed::create_from_mubook($mubook, $context)->trigger();
        $events = $sink->get_events();
        $sink->close();

        $this->assertCount(1, $events);
        $this->assertInstanceOf(all_chapters_viewed::class, $events[0]);

        $event = $events[0];
        $this->assertEquals($context->id, $event->contextid);
        $this->assertSame($mubook->id, $event->objectid);
        $this->assertSame('r', $event->crud);
        $this->assertSame($event::LEVEL_PARTICIPATING, $event->edulevel);
        $this->assertSame('mubook', $event->objecttable);
        $this->assertSame('All chapters page viewed', $event::get_name());
        $description = $event->get_description();
        $tenanturl = new \core\url('/mod/mubook/viewall.php', ['id' => $mubook->cmid]);
