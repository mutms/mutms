...
/**
 * Provides list of candidates for supervisor position when creating a team.
 *
 * @package     tool_murelation
 * @copyright   2025 Petr Skoda
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class team_create_userid extends \tool_mulib\external\form_autocomplete\user {
    #[\Override]
    ...
    public static function execute(string $query, int $frameworkid, ?int $tenantid): array {
        ...
        $sql = new sql(
            "SELECT usr.*
               FROM {user} usr
               /* cohortjoin */
              WHERE usr.deleted = 0 AND usr.confirmed = 1 AND usr.suspended = 0
                    /* search */ /* tenant */
            /* orderby */"
        );
