...
/**
 * Provides list of subordinate candidates for new team.
 *
 * @package     tool_murelation
 * @copyright   2025 Petr Skoda
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class team_create_subuserids extends \tool_mulib\external\form_autocomplete\user {
    #[\Override]
    ...
    public static function execute(string $query, int $frameworkid, ?int $tenantid): array {
        ...
        if (!\tool_murelation\local\uimode_teams::can_create_team($framework, $context)) {
    ...
    public static function validate_value(int $value, array $args, \context $context): ?string {
        ...
        if ($DB->record_exists('tool_murelation_subordinate', ['frameworkid' => $framework->id, 'userid' => $user->id])) {
            return get_string('error');
        }
